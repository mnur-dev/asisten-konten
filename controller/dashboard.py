...
def add_routes(app, db, inputs, results):
    ...
    def create_job(pgn_file: UploadFile, timestamps_file: UploadFile | None = None, authorization: str | None = Header(None)):
        ...
        connection = db(); connection.execute("INSERT INTO jobs(id,status,created_at,pgn_path,timestamps_path) VALUES(?,?,?,?,?)", (jid, "WAITING", datetime.now(timezone.utc).isoformat(), str(path), str(timestamps_path) if timestamps_path else None)); connection.commit()
